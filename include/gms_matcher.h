...
// 5 level scales
const double mScaleRatios[5] = {1.0, 1.0 / 2, 1.0 / std::sqrt(2.0),
                                std::sqrt(2.0), 2.0};

class GMSMatcher {
public:
  // OpenCV Keypoints & Correspond Image Size & Nearest Neighbor Matches
  GMSMatcher(const std::vector<KeyPoint> &vkp1, const Size size1,
...
private:
  ...
  void NormalizePoints(const std::vector<KeyPoint> &kp, const Size &size,
                       std::vector<Point2f> &npts) {
  ...
  void ConvertMatches(const std::vector<DMatch> &vDMatches,
...
int GMSMatcher::GetInlierMask(std::vector<bool> &vbInliers, bool WithScale,
                              bool WithRotation) {
...
void matchGMS(const Size &size1, const Size &size2,
              ...
              std::vector<DMatch> &matchesGMS, const bool withRotation = false,
              const bool withScale = false, const double thresholdFactor = 6.0) {
  GMSMatcher gms(keypoints1, size1, keypoints2, size2, matches1to2,
                 thresholdFactor);
